...
async def deliver_whatsapp_twilio(payload: Dict[str, Any]) -> ChannelResult:
    sid = (
        payload.get("_twilio_account_sid") or settings.twilio_account_sid or ""
    ).strip()
    auth = (
        payload.get("_twilio_auth_token") or settings.twilio_auth_token or ""
    ).strip()
    from_wa = (
        payload.get("_twilio_whatsapp_from")
        or settings.twilio_whatsapp_from
        or ""
    ).strip()
    to = (
        payload.get("whatsapp_to") or payload.get("to_whatsapp") or ""
    ).strip()
    ...
    data = {
        "From": from_wa,
        "To": to,
        "Body": body_text or (payload.get("title") or "Notification"),
    }
    try:
