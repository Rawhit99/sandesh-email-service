from sqlalchemy import Boolean, create_engine, Column, Integer, String, DateTime, Text, JSON, Index, ForeignKey, UniqueConstraint
...
class Organization(Base):
    ...
    service_user_id = Column(Integer, ForeignKey("users.id", ondelete="SET NULL"), nullable=True, unique=True, index=True)
...
class Subscriber(Base):
    """Novu-style subscriber; deliveries can be gated on this row."""
    ...
    __table_args__ = (UniqueConstraint("user_id", "subscriber_id", name="uq_subscriber_user_ext"),)
...
class IntegrationCredential(Base):
    ...
    Users can store multiple credential profiles per channel (e.g. two AWS SES accounts)
    and optionally designate one as the default. When sending notifications the caller
    can reference a non-default profile by passing ``credential_name`` in the payload.
    """
    ...
    channel = Column(String(60), nullable=False)   # e.g. "aws_ses", "sns", "slack", "ms_teams", "smtp"
    name = Column(String(120), nullable=False)      # user-assigned label, e.g. "production-ses"
    config = Column(JSONB, nullable=False, server_default='{}')  # channel-specific key/value pairs
    ...
    __table_args__ = (
        # Each (user, channel, name) triplet must be unique
        UniqueConstraint("user_id", "channel", "name", name="uq_intcred_user_channel_name"),
        Index("ix_intcred_user_channel", "user_id", "channel"),
    )


class OrgTemplateSetting(Base):
    """Controls whether a template (by template_id string) is enabled for an org.

    Default behaviour: no row means the template IS enabled for that org.
    A row with is_enabled=False disables access/sends for that org.
    """
