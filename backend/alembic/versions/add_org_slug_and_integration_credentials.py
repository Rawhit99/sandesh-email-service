...
def upgrade():
    ...
    if not _column_exists(conn, "organizations", "org_slug"):
        op.add_column(
            "organizations",
            sa.Column("org_slug", sa.String(60), nullable=True),
        )

    if not _constraint_exists(
        conn, "organizations", "uq_organizations_org_slug"
    ):
        op.create_unique_constraint(
            "uq_organizations_org_slug", "organizations", ["org_slug"]
        )

    if not _index_exists(conn, "organizations", "ix_organizations_org_slug"):
        op.create_index(
            "ix_organizations_org_slug",
            "organizations",
            ["org_slug"],
            unique=True,
        )
    ...
    if not _index_exists(
        conn, "integration_credentials", "ix_intcred_user_channel"
    ):
        op.create_index(
            "ix_intcred_user_channel",
            "integration_credentials",
            ["user_id", "channel"],
        )

    if not _constraint_exists(
        conn, "integration_credentials", "uq_intcred_user_channel_name"
    ):
        op.create_unique_constraint(
...
def downgrade():
    ...
    if _index_exists(
        ...
        op.drop_index(
            "ix_intcred_user_channel", table_name="integration_credentials"
        )
    ...
    if _constraint_exists(conn, "organizations", "uq_organizations_org_slug"):
        op.drop_constraint(
            "uq_organizations_org_slug", "organizations", type_="unique"
        )
